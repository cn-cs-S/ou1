...
export default function TradingTerminal() {
  ...
  const refreshTerminal = useCallback(async () => {
    ...
    if (healthResult.status === 'fulfilled') {
      ...
      if (!accountSelectionInitializedRef.current) {
        ...
        if (healthResult.value.accountSource === 'live-readonly') {
          setSelectedAccountSource('live-readonly')
          setSelectedAccountId('live-readonly')
        }
      }
  ...
  function toggleFavorite(instId: string) {
    setFavoriteIds((current) => {
      ...
      void saveFavorites(nextFavorites).catch(() => {
        setActionError('自选币种已保存在本机浏览器，但同步到本地服务失败。')
      })
      return next
    })
  }
