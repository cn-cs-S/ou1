...
class GlmChatOpenAI(NormalizedChatOpenAI):
    """GLM/Zhipu-specific overrides for switchable thinking mode.

    Zhipu's OpenAI-compatible chat endpoint accepts ``thinking={"type": ...}``
    for GLM 4.5+ / 4.7+ models. The user can set ``ZHIPU_THINKING_TYPE`` to
    ``enabled`` or ``disabled``; defaulting to disabled keeps saved reports
    compact and avoids leaking reasoning blocks into downstream agents.
    """

    def _get_request_payload(self, input_, *, stop=None, **kwargs):
        payload = super()._get_request_payload(input_, stop=stop, **kwargs)
        thinking_type = os.environ.get("ZHIPU_THINKING_TYPE", "disabled").strip().lower()
